...
describe("rollup", () => {
  it(
    "generates all server files (string input)",
    { timeout: 30000 },
    async () => {
      ...
      const handler = gen.output.find(
        (f: any) => f.facadeModuleId === entry,
      ) as OutputChunk | undefined;
      expect(handler?.name).toEqual(
        join("test", "files", "folder1", "handler"),
      );
  ...
  it(
    ...
    async () => {
      ...
      expect(handler2?.name).toEqual(
        join("test", "files", "folder2", "handler"),
      );
